...
// --- Helper functions and type definitions (for context) ---

/**
 * Gets the application's default timezone from environment variables.
 * Assumes a .env file with a variable like:
...
/**
 * Finds the most recent pending appointment for a given user and doctor
 * that has not yet expired.
 *
 * @param {PendingAppointmentParams} { userId, doctorId } - The user and doctor IDs.
 * @returns {Promise<ServerActionResponse<PendingAppointmentData>>} The server action response.
 */
...
> {
  try {
    ...
    return {
      success: false,
      message: "Could not retrieve appointment pending details",
